...
static Tree CreateExternalNode(Ingredient key, int id_doc) {
    ...
    p->node.item.name = strdup(key.name); // alocação e copia da palavra para o nó externo
...
static void Print_Node(Ingredient node){
    /*Função axuliar para impressão da lista de indices invertido de uma palavra
     * */
