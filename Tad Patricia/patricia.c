...
void InsertFromFile(const char *filename, Tree *patricia) {
...
void SearchFromFile(const char *filename, Tree *patricia) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        perror("Unable to open file");
        return;
...
int main(void) {
    ...
    InsertFromFile("words2.txt", &patricia);

    Ingredient tst;
    tst.name = "egg";
    patricia = Insert(tst, &patricia);

    print_tree(patricia);
    printf("Com = %d\n", con2);

    SearchFromFile("words2.txt", &patricia);
