...
#include <math.h>
#include "patricia.h"
#define N 15  // Colocar a quantidade de documentos


// LISTA ENCADEADA
static index_Node_p *initialize_indexNode_p(int documentID) {
    /* Função responsável pela inicialização da lista de ocorrência
       de um determinado ingrediente de um determinado doc */
    index_Node_p *i_node = (index_Node_p*)malloc(sizeof(index_Node_p));
    if (i_node == NULL) {
...
static void insert_new_occurence(Occurences list, int documentID) {
    ...
    */
    ...
    if (new_node != NULL) {
        ...
        new_node = initialize_indexNode_p(documentID);
        if (new_node == NULL) {
...
static Tree CreateExternalNode(Ingredient key, int id_doc) {
    ...
    strncpy(p->node.item.name, key.name, 50); // copiar a string para o array
    p->node.item.list = (Occurences)malloc(sizeof(index_Node_p)); // alocação de memoria para a lista de indices invertidos
    if (p->node.item.list == NULL) {
        printf("\nErro ao alocar memória para a lista de índices\n");
        return NULL;
    }
    p->node.item.list = initialize_indexNode_p(id_doc); // criação da lista
    p->type = EXTERNAL;
    return p;
}
...
Tree Insert(Ingredient key, Tree *p, int *com, int id_doc, int command) {
    ...
    */
    ...
    if (command == 1) {
        printf("Palavra %s inserida na patricia, foram necessaria(s) %d comparacoes\n", key.name, *com);
    }
    (*com) = 0;
    return temp;
}
...
void tf_idf(char *words[], Tree patricia, int qtd_terms){ //Calcula a relevancia dos arquivos para aquelas palavras
    ...
    for(int i = N; i > 0; i--){ //Imprime documentos mais relevantes, em que as palavras aparecem
