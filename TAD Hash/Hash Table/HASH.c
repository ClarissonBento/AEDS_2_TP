...
void insert_in_hashtable(Hash_Table *table, char *word, int id_doc, int *weights, int M) {
    int hashcode = hash(word, weights, M);
    Node *new_node = createNode(word, id_doc);

    if (new_node == NULL) {
        printf("\nErro ao criar um novo node\n");
        return;
    }
    ...
    if (is_emptyList(table->linear_list[hashcode])) {
        ...
        Node *current = table->linear_list[hashcode]->tail;
